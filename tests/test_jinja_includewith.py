...
def test_set_context():
    tmpl = env.from_string('''{% include "hello" with what='world' %}''')
...
def test_multiple_values():
    tmpl = env.from_string('''{% include "sum" with a=3, b=2, c=3+2 %}''')
    expected = '''3 + 2 makes 5'''
    result = tmpl.render()
    assert result == expected


def test_careless_formatting():
    tmpl = env.from_string('''{% include "sum" with a = 'Antartica', b=42,c='no sense' %}''')
    expected = '''Antartica + 42 makes no sense'''
    result = tmpl.render()
    assert result == expected


def test_include_current_context():
    tmpl = env.from_string('''{% set a = 2 %}{% include "sum" with c=4 %}''')
    expected = '''2 + 2 makes 4'''
